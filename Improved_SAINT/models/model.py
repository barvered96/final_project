        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    PostNorm(dim, Residual(Attention(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout))),
                    PostNorm(dim, Residual(FeedForward(dim, dropout = ff_dropout))),
                    PostNorm(dim*nfeats, Residual(Attention(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout))),
                    PostNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
